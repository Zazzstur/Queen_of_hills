# Deployment Guide: Connecting Cloudflare Pages to Supabase

Your website is built with Vite and uses Supabase for its backend. When deploying to Cloudflare Pages (or Vercel/Netlify), the environment variables stored in your local `.env` file are **not** automatically uploaded. You must manually configure them in your hosting provider's dashboard.
